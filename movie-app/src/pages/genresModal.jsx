...
function GenresModal({data,setShowModal,setGenre,setGenreName}) {
  ...
  return (
    <div >
        <div className='bg-black opacity-[0.9] blur-[3] absolute top-0 bottom-0 left-0 right-0  flex z-[1] justify-center pt-[50px] transition-all ease-in-out '>
        <ul className=" flex justify-center items-center flex-col ">{data?.map((genreName,i)=>
            (<li key={i} className=' cursor-pointer list-none font-semibold text-white text-[18px] hover:bg-slate-600 max-[768px]:active:bg-slate-800 px-[10px] max-[768px]:cursor-default rounded-[5px]' onClick={()=>{setGenre(genreName.id) 
            setShowModal(false)
